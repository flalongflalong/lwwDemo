d3.biHiSankey = function () {
    ...
    function computeNodeHierarchy() {
        ...
        nodes.forEach(function (node) {
            ...
            node.x = 0;
            node.y = 0
            node.childMaxX = parseInt(node.childMaxX?node.childMaxX:1);
            node.childMaxY = parseInt(node.childMaxY?node.childMaxY:1);
        ...
        nodes.forEach(function (node) {
            ...
            if (node.children.length == 0) {
                node.wc = node.childMaxX;
                node.hc = node.childMaxY;
            } else {
                ...
                nodesCByXPosition.forEach(function (ds,xi) {
                    ...
                    if (xi>0) {
                        node.xc += (nodeSpacingArray[node.slevel + 1] * 5);
                    }
                ...
                nodesCByYPosition.forEach(function (ds,yi) {
                    ...
                    if (yi>0) {
                        node.yc += nodeSpacingArray[node.slevel + 1];
                    }
                })
            }
        });
    ...
    function computeNodeValues() {
        ...
        nodes.forEach(function (node) {
            ...
            // node.height = node.childMaxY * setNodeHeight ;
    ...
    function compressInXDirection() {
        ...
        nodesByXPosition.forEach(function (xnodes,iii,jjj) {
            ...
            function xnodeConfig (xnodes) {
                ...
                xnodes.forEach(function (node,i,j) {
                    ...
                    node.width = ( node.wc * nodeWidth + node.xc );
                    node.height = (node.hc * setNodeHeight + node.yc );

                    if (node.slevel<2) {
                        node.width += nodeSpacing*(3-node.slevel)
                    }
                    if (node.slevel<2) {
                        node.height += nodeSpacing*(2-node.slevel)
                    }
                    if (node.slevel == 0) {
                        node.height += nodeSpacing*(node.childMaxY - 1)
                    }
                    ...
                    node.x += ((prevXnode.length>0)?(prevXnode[0].x + prevXnode[0].width + nodeSpacingArray[node.slevel]*5):nodeSpacing);
                    node.y += ((prevYnode.length>0)?(prevYnode[0].y + prevYnode[0].height + nodeSpacingArray[node.slevel]):nodeSpacing);
    ...
    function scaleNodeXPositions() {
        var minX = d3.min(nodes, function (node) { return node.x; }),
            childMaxX = d3.max(nodes, function (node) { return node.x; }) - minX;
        xScaleFactor = (size[0] - nodeWidth) / childMaxX;
    ...
    function computeNodeYPositions(iterations) {
        ...
        function initializeNodeYPosition() {
            nodesByXPosition.forEach(function (nodes) {
                nodes.forEach(function (node, i) {
                    ...
                    node.heightAllowance = node.childMaxY * yScaleFactor + linkSpacing * node.linkSpaceCount;
                });
            });
        }
        ...
        function calculateLinkThickness() {
            links.forEach(function (link) {
                ...
                // link.thickness = 10
                // var rh = link.source.height / (link.source.rightLinks.filter(function (node) { return node.target.state === "collapsed"; })).length;
                // var lh = link.target.height / (link.target.leftLinks.filter(function (node) { return node.source.state === "collapsed"; })).length;
                // rh = (rh != Infinity)?(rh/2):30;
                // lh = (lh != Infinity)?(lh/2):30;
                // link.thickness = d3.min([rh,lh])
            ...
            nodesByXPosition.slice().reverse().forEach(function (nodes) {
                nodes.forEach(function (node) {
                    if (node.leftLinks.length) {
                        ...
                        // node.y = node.py * (setNodeHeight + nodeSpacing)
                    }
                });
            });
        }
        ...
        function resolveCollisions() {
            ...
            nodesByXPosition.forEach(function (nodes) {
                ...
                function  gIdx (nodeA, nodeB) {
                    var idxArray = [];
                    nodeB.children.forEach(function (nn) {
                        idxArray.push(nn.id+"")
                    })
                    return idxArray.indexOf((nodeA.id+"")) ;
                };
                ...
                for (i = 0; i < n; ++i) {
                    ...
                    if (dy > 0) {
                        if (node.state == 'collapsed') {
                            ...
                            var _idx = (gIdx(node,node.parent)!=-1)?gIdx(node,node.parent):0;
                            node.y = node.parent.y + _idx*40 + nodeMargin;
                        }
                    }
    ...
    biHiSankey.setNodeHeight = function (_) {
        if (!arguments.length) { return setNodeHeight; }
        setNodeHeight = +_;
        return biHiSankey;
    };
